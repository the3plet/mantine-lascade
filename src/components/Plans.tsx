    ...
    <div className="py-8">
      ...
      <div className="lg:grid lg:grid-cols-3 gap-1">
        {api.plans.map((plan) => (
          <Card
            key={plan.id}
            className={`mx-14 my-6 rounded-4xl lg:w-sm ${
              plan.name === "Weekly"
            ...
            <CardContent className="">
              ...
              {Object.values(plan.features).map((feature, index) => (
                <div key={index} className="flex items-center gap-2 mb-4">
                  <Checkbox className="w-4 h-4 flex justify-center items-center data-[state=checked]:bg-blue-500 data-[state=checked]:text-black  "checked={index !=5} />
                  <p className="flex items-center text-sm">{feature}</p>
                </div>
              ))}
              <h1 className="text-xl font-semibold">{plan.price}</h1>
              <Button
                className={`w-full rounded-2xl bg-[#171717] border-blue-400 text-blue-400 !mb-0 my-4 ${
                  plan.name === "Weekly"
